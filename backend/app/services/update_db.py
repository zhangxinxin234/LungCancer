...
def data_loader_save_as_dict(raw_data_path, colletion_type):
    ...
    for document_infos in all_data_list:
        try:
            ...
            for content in document_infos.get("分层内容", ""):
                if content.get("参考id", ""):
                    for refer_id in content.get("参考id", "").split("，"):
                        doc_content.append(
                            {all_data_dict[refer_id].get("分期", ""): all_data_dict[refer_id].get("分层内容", "")})
...
if __name__ == '__main__':
    dir_test = "data/rag/csco 指南 (7).jsonl"
